...
static void mg_sendnsreq(struct mg_connection *c, struct mg_str *name, int ms,
    ...
    if (dnsc->c == NULL) {
        ...
        MG_VERBOSE(("%lu resolving %.*s @ %s, txnid %hu", c->id, (int)name->len,
                    name->ptr, mg_ntoa(&dnsc->c->rem, buf, sizeof(buf)),
                    d->txnid));
...
void mg_http_serve_file(struct mg_connection *c, struct mg_http_message *hm,
    ...
    if (fd == NULL || fs->st(path, &size, &mtime) == 0) {
        ...
        if (mg_vcasecmp(&hm->method, "HEAD") == 0) {
            ...
            c->pfn_data = fd;
            *(size_t *)c->label =
                (size_t)cl;  // Track to-be-sent content length
        }
    }
}
    ...
    if (ev == MG_EV_READ || ev == MG_EV_CLOSE) {
        ...
        while (c->recv.buf != NULL && c->recv.len > 0) {
            ...
            if (ev == MG_EV_CLOSE) {  // If client did not set Content-Length
                ...
                hm.body.len =
                    hm.message.len - (size_t)(hm.body.ptr - hm.message.ptr);
            }
            if (mg_is_chunked(&hm)) {
                ...
                switch (mm.cmd) {
                    case MQTT_CMD_CONNACK:
                        ...
                        if (mm.ack == 0) {
                            ...
                            MG_ERROR(("%lu MQTT auth failed, code %d", c->id,
                                      mm.ack));
                            c->is_closing = 1;
                        }
                        break;
                    case MQTT_CMD_PUBLISH: {
                        ...
                        if (mm.qos > 0) {
                            ...
                            mg_mqtt_send_header(c, MQTT_CMD_PUBACK, 0,
                                                sizeof(id));
...
void mg_rpc_process(struct mg_rpc_req *r) {
    ...
    if (off > 0 && r->frame.ptr[off] == '"') {
        struct mg_str method =
            mg_str_n(&r->frame.ptr[off + 1], (size_t)len - 2);
...
void mg_connect_resolved(struct mg_connection *c) {
    ...
    if (FD(c) == INVALID_SOCKET) {
        ...
        if ((rc = connect(FD(c), &usa.sa, slen)) == 0) {
            ...
            MG_DEBUG(("%lu %p -> %x:%hu pend", c->id, c->fd,
...
static void accept_conn(struct mg_mgr *mgr, struct mg_connection *lsn) {
    ...
    if (fd == INVALID_SOCKET) {
        ...
        // readable even it is not. See comment for 'select' func implementation
        // in nx_bsd.c That's not an error, just should try later
    ...
    (MG_ARCH != MG_ARCH_TIRTOS) && !(MG_ENABLE_POLL)
        ...
        MG_DEBUG(("%lu %p accepted %x.%hu -> %x.%hu", c->id, c->fd,
                  mg_ntohl(c->rem.ip), mg_ntohs(c->rem.port),
                  mg_ntohl(c->loc.ip), mg_ntohs(c->loc.port)));
        mg_call(c, MG_EV_OPEN, NULL);
    ...
    while (i < p.len || j < s.len) {
        if (i < p.len && j < s.len &&
            ...
            if (caps == NULL) {
                ...
                caps->len =
                    (size_t)(&s.ptr[j] - caps->ptr);  // Finalize current cap
...
void mg_tls_init(struct mg_connection *c, const struct mg_tls_opts *opts) {
    ...
    if (opts->ca == NULL || strcmp(opts->ca, "*") == 0) {
        ...
        if (opts->srvname.len > 0) {
            char *x =
                mg_mprintf("%.*s", (int)opts->srvname.len, opts->srvname.ptr);
...
int mg_check_ip_acl(struct mg_str acl, uint32_t remote_ip) {
    struct mg_str k, v;
    int allowed =
        acl.len == 0 ? '+' : '-';  // If any ACL is set, deny by default
        ...
        while (ws_process(c->recv.buf + ofs, c->recv.len - ofs, &msg) > 0) {
            ...
            switch (op) {
                ...
                case WEBSOCKET_OP_CLOSE:
                    ...
                    mg_call(c, MG_EV_WS_CTL, &m);
                    // Echo the payload of the received CLOSE message back to
                    // the sender
...
struct stm32_eth {
    volatile uint32_t MACCR, MACFFR, MACHTHR, MACHTLR, MACMIIAR, MACMIIDR,
        ...
        MMCTGFCR, RESERVED5[10], MMCRFCECR, MMCRFAECR, RESERVED6[10], MMCRGUFCR,
...
static size_t mip_driver_stm32_tx(const void *buf, size_t len, void *userdata) {
    if (len > sizeof(s_txbuf[s_txno])) {
        ...
        s_txdesc[s_txno][0] =
            BIT(20) | BIT(28) | BIT(29) | BIT(30);  // Chain,FS,LS
...
static uint8_t *arp_cache_find(struct mip_if *ifp, uint32_t ip) {
    ...
    for (uint8_t i = 0, j = p[1]; i < MIP_ARP_ENTRIES; i++, j = p[j + 1]) {
        if (memcmp(p + j + 2, &ip, sizeof(ip)) == 0) {
            ...
            // MG_DEBUG(("ARP find: %#lx @ %x:%x:%x:%x:%x:%x", (long) ip, p[j +
            // 6],
...
static void rx_dhcp(struct mip_if *ifp, struct pkt *pkt) {
    ...
    if (ip && mask && gw && ifp->ip == 0) {
        arp_cache_add(ifp, pkt->dhcp->siaddr,
                      ((struct eth *)pkt->raw.buf)->src);
...
static size_t tx_tcp_pkt(struct mip_if *ifp, struct pkt *pkt, uint8_t flags,
    ...
    return tx_tcp(ifp, pkt->ip->src, flags, pkt->tcp->dport, pkt->tcp->sport,
                  seq, mg_htonl(mg_ntohl(pkt->tcp->seq) + delta), buf, len);
}
...
static void mip_poll(struct mip_if *ifp, uint64_t uptime_ms) {
    ...
    for (;;) {
        size_t len =
            ifp->queue.len > 0
                ? q_read(&ifp->queue, ifp->rx.buf)
                : ifp->driver->rx(ifp->rx.buf, ifp->rx.len, ifp->driver_data);
        if (len == 0) break;
